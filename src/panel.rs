...
//! interactive (`type: "controls"` — requires aura >= 0.1.26); every
...
fn build() -> anyhow::Result<Value> {
    ...
    for (name, profile) in &config.profiles {
        let mut header_buttons: Vec<Value> = KNOWN_EVENTS
            ...
            .map(|e| button(format!("event:add:{name}:{e}"), &format!("+ {e}"), false, false))
            .collect();
        header_buttons.push(button(format!("profile:rm:{name}"), "Delete", false, true));
        profile_controls.push(json!({
            "label": format!("▸ {name}"),
            "hint": format!("{} event(s)", profile.events.len()),
        ...
        for (event, source) in &profile.events {
            ...
            let detail = if path.is_file() {
                ...
                let n = std::fs::read_dir(&path).map(|d| d.flatten().count()).unwrap_or(0);
                format!("{n} files")
            } else {
                "missing!".to_string()
            };
            profile_controls.push(json!({
                ...
                "buttons": [
                    ...
                    button(format!("event:rm:{name}:{event}"), "✕", false, true),
                ],
            }));
        }
    }
